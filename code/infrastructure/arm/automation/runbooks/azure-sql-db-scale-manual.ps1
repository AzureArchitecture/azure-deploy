...
workflow azure-sql-db-scale-manual
{
    param
    (
        # Name of the Azure SQL Database server (Ex: bzb98er9bp)
    ...
    inlinescript
    {
        ...
        $CTX = New-AzureSqlDatabaseServerContext -ManageUrl â€œhttps://$Using:SqlServerName.database.windows.netâ€ -Credential $ServerCredential
        
        # Get Azure SQL Database context
        $Db = Get-AzureSqlDatabase $CTX â€“DatabaseName $Using:DatabaseName
        ...
        Set-AzureSqlDatabase $CTX â€“Database $Db â€“ServiceObjective $ServiceObjective â€“Edition $Using:Edition -Force
