...
function AssertResourceManagerVirtualMachinePowerState
{
    ...
    $currentestatus = $resourceManagerVM.Statuses | where Code -like "PowerState*" 
    $currentestatus = $currentestatus.Code -replace "PowerState/",""

    # If should be started and isn't, start VM
	if($DesiredState -eq "Started" -and $currentestatus -notmatch "running")
	{
        if($Simulate)
        {
            Write-Output "[$($VirtualMachine.Name)]: SIMULATION -- Would have started VM. (No action taken)"
	...
	# If should be stopped and isn't, stop VM
	elseif($DesiredState -eq "StoppedDeallocated" -and $currentestatus -ne "deallocated")
	{
        if($Simulate)
        {
            Write-Output "[$($VirtualMachine.Name)]: SIMULATION -- Would have stopped VM. (No action taken)"
    ...
    {
        Write-Output "[$($VirtualMachine.Name)]: Current power state [$currentestatus] is correct."
    }
}
