...
Param(
    [string] [Parameter(Mandatory = $true)] $ArtifactestagingDirectory,
    [string] [Parameter(Mandatory = $true)][alias("ResourceGroupLocation")] $Location,
    [string] $ResourceGroupName = (Split-Path $ArtifactestagingDirectory -Leaf),
    [switch] $UploadArtifacts,
    [string] $StorageAccountName,
    [string] $StorageContainerName = $ResourceGroupName.ToLowerInvariant() + '-stageartifacts',
    [string] $TemplateFile = $ArtifactestagingDirectory + '\mainTemplate.json',
    [string] $TemplateParametersFile = $ArtifactestagingDirectory + '.\azuredeploy.parameters.json',
    [string] $DSCSourceFolder = $ArtifactestagingDirectory + '.\DSC',
...
$ArtifactestagingDirectory = ($ArtifactestagingDirectory.TrimEnd('/')).TrimEnd('\')

# if the template file isn't found, try the another default
if (!(Test-Path $TemplateFile)) { 
    $TemplateFile = $ArtifactestagingDirectory + '\azuredeploy.json'
}
...
if ($UploadArtifacts -Or $ArtifactsLocationParameter -ne $null) {
    ...
    $ArtifactestagingDirectory = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($PSScriptRoot, $ArtifactestagingDirectory))
    ...
    $ArtifactestagingLocation = $StorageAccount.Context.BlobEndPoint + $StorageContainerName + "/"   
    ...
    if ($OptionalParameters[$ArtifactsLocationName] -eq $null) {
        ...
        if ($defaultValue -like '*deployment().properties.templateLink.uri*') {
            ...
            $OptionalParameters[$ArtifactsLocationName] = $ArtifactestagingLocation   
        }
    } 
    ...
    $ArtifactFilePaths = Get-ChildItem $ArtifactestagingDirectory -Recurse -File | ForEach-Object -Process { $_.FullName }
    ...
    $TemplateArgs.Add('TemplateUri', $ArtifactestagingLocation + (Get-ChildItem $TemplateFile).Name + $OptionalParameters[$ArtifactsLocationSasTokenName])
