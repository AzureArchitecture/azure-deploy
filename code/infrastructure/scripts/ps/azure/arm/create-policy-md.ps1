...
document 'arm-template' {
    $metadata = Get-TemplateMetadata -Path $env:METADATA_PATH/policy.metadata.json;
    $rules = Get-TemplateResources -Path  $env:TEMPLATE_PATH/policy.rules.json;
    $policyConfig = Get-TemplateResources -Path  $env:TEMPLATE_PATH/policy.config.json;
    $parameters = Get-Content -Path $env:TEMPLATE_PATH/policy.parameters.json;
...
'
    ...
    # Set document title
    Title "Policy Definition: $env:templateName"
   ...
   Section 'File Details' {
      'Config File: [policy.config.json](' +  $env:TEMPLATE_PATH + '/policy.config.json + )'
      'Rules File: [policy.rules.metadata.json](' +  $env:TEMPLATE_PATH + 'policy.rules.json + )'
      'Metadata File: [policy.metadata.json](' +  $env:METADATA_PATH + '/policy.metadata.json + )'
      'Parameters File: [policy.parameters.json](' +  $env:TEMPLATE_PATH + '/policy.parameters.json + )'
    }
